...
class CrosswordCreator():
    ...
    def revise(self, x, y):
        ...
        for wx in self.domains[x]:
            # Check if there exists some wy in domain(y) that is compatible
            if not any(wx[i] == wy[j] for wy in self.domains[y]):
                to_remove.add(wx)
    ...
    def consistent(self, assignment):
        ...
        # Correct length for each variable
        for var, word in assignment.items():
            if len(word) != var.length:
                return False

        # No conflicts on overlaps
        for var, word in assignment.items():
            for neigh in self.crossword.neighbors(var):
                if neigh not in assignment:
                    continue
                overlap = self.crossword.overlaps.get((var, neigh))
                if overlap is None:
                    continue
                i, j = overlap
        ...
        def ruled_out_count(value):
            ...
            for n in self.crossword.neighbors(var):
                ...
                if overlap is None:
                    continue
                i, j = overlap
                # For each neighbor value that conflicts, increment count
                for nv in self.domains[n]:
                    if value[i] != nv[j]:
                        count += 1
            return count

        # Sort values by least number of ruled-out neighbor values
    ...
    def backtrack(self, assignment):
        ...
        for value in self.order_domain_values(var, assignment):
            ...
            if self.consistent(new_assignment):
                # Optional inference could go here (e.g., AC-3 with arcs from var)
